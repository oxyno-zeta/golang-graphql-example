		...
		for d := range deliveries {
			...
			if err != nil {
				...
				if consumeCfg.RequeueOnNackFn != nil {
					requeue = consumeCfg.RequeueOnNackFn(&d)
				}
...
func (as *amqpService) extractTraceFromHeaders(h amqp091.Table) (tracing.Trace, error) {
	...
	for k, v := range h {
		...
		switch v.(type) { // nolint: gocritic // Ignore because can't do this in if
		case string:
			headers[k] = v.(string) // nolint: forcetypeassert // Type already checked
		}
	}
