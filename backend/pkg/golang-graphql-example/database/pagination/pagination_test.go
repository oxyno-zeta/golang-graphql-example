...
func TestPaging(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			countExpectedArgs:               []driver.Value{},
			countResult:                     3,
			selectExpectedIntermediateQuery: "ORDER BY created_at DESC LIMIT $1",
			selectExpectedArgs:              []driver.Value{10},
		...
		},
		{
			...
			countResult:                     3,
			selectExpectedIntermediateQuery: "ORDER BY created_at DESC LIMIT $1",
			selectExpectedArgs:              []driver.Value{5},
			selectExpectedProjectionQuery:   "*",
		...
		},
		{
			...
			countExpectedArgs:               []driver.Value{},
			countResult:                     30,
			selectExpectedIntermediateQuery: "ORDER BY created_at DESC LIMIT $1 OFFSET $2",
			selectExpectedArgs:              []driver.Value{5, 20},
			selectExpectedProjectionQuery:   "*",
		...
		},
		{
			...
			countResult:                     30,
			selectExpectedIntermediateQuery: "WHERE name = $1 ORDER BY name DESC LIMIT $2 OFFSET $3",
			selectExpectedArgs:              []driver.Value{"fake", 5, 20},
			selectExpectedProjectionQuery:   "*",
			want:                            &PageOutput{TotalRecord: 30, Limit: 5, Skip: 20, HasNext: true, HasPrevious: true},
		},
		{
		...
		},
		{
			...
			countResult:                     30,
			selectExpectedIntermediateQuery: "WHERE fake = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3",
			selectExpectedArgs:              []driver.Value{"fake1", 5, 20},
			selectExpectedProjectionQuery:   "*",
			want:                            &PageOutput{TotalRecord: 30, Limit: 5, Skip: 20, HasNext: true, HasPrevious: true},
		},
		{
